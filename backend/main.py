...
async def generate_answer(query: str, hits: list[dict], model: Optional[str] = None) -> str:
    ...
    context_block = "\n\n".join(blocks)
    prompt = (
        "You are a helpful assistant. Answer the question using ONLY the context below. Do not use prior knowledge.\n\n"
        "The retrieved context may contain adjacent, related, or unrelated information from the same source, including neighboring FAQ sections, navigation text, boilerplate, marketing language, and unrelated material caused by chunk boundaries. Treat the retrieved context as an evidence pool, not as material that must all be summarized.\n\n"
        "Relevance and Scope\n"
        "- Answer only the user's question.\n"
        "- Do not include a fact simply because it appears in the retrieved context.\n"
        "- Before including each factual claim, verify:\n"
        "  1. The claim is explicitly supported by the retrieved evidence (grounding test).\n"
        "  2. The claim is directly relevant and useful for answering the user's question (relevance test).\n"
        "- A claim must pass BOTH tests. If either is false, omit the claim.\n"
        "- Prefer a concise, complete answer over a broad summary of everything retrieved.\n"
        "- Do not introduce tangential facts with phrases such as 'It's worth noting...', 'Additionally...', 'Relatedly...', or 'You may also want to know...' unless the additional information is materially necessary to answer the question or prevent a significant misunderstanding.\n\n"
        "Question Intent\n"
        "- Identify the user's primary information need before composing the answer.\n"
        "- Use that intent to determine which retrieved claims belong in the response.\n"
        "- For example, if the user asks about Princess Cruises' smoking policy, include only smoking-related facts (where permitted, prohibited, vaping/e-cigarette rules, balcony/stateroom restrictions, fines, safety requirements directly related to smoking). Do not include drinking ages, casino age restrictions, dress codes, unrelated accessibility policies, or other nearby FAQ topics, even if they appear in the same source.\n\n"
        "Evidence Is Necessary but Not Sufficient\n"
        "- Source support alone does not make a claim appropriate.\n"
        "- Use this rule:\n"
        "  include_claim = supported_by_evidence AND relevant_to_question\n"
        "- Do not use:\n"
        "  include_claim = supported_by_evidence\n\n"
        "Avoid Adjacent-Chunk Leakage\n"
        "- When a retrieved chunk contains multiple topics, identify which portion answers the user's question, use only that portion as answer evidence, and ignore neighboring sections unless they materially affect the answer.\n"
        "- Do not summarize the entire chunk.\n\n"
        "Preserve Important Details\n"
        "- Relevance filtering should not make answers overly terse.\n"
        "- Include all material details needed to answer the question accurately (exceptions, restrictions, penalties, qualifications, conditions, important distinctions).\n"
        "- Do not remove a detail merely to shorten the response if it materially changes the meaning.\n\n"
        "Citations and Answer Style\n"
        "- Synthesize the relevant information into a clear, flowing answer. Do not repeat the same fact or restate the question.\n"
